...
// getStore is loaded via createRequire (Node built-in) so this stays valid ESM
// on Netlify without a static bare import a frontend bundler would resolve.
...
export async function saveRegistration(reg) {
  ...
  await store.setJSON(`reg:${reg.token}`, { ...reg, updated_at: new Date().toISOString() });
}
...
export async function listRegistrations() {
  ...
  for (const b of blobs) {
    try {
      ...
      if (reg && reg.token) regs.push(reg);
    } catch {
